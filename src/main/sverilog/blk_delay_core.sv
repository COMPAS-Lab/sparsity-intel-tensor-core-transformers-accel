...
module blk_delay_core #( parameter
...
generate
  ...
  if ( LENGTH == 0 ) begin
    ...
    assign dat_out[WIDTH-1:0] = dat_in[WIDTH-1:0];
    ...
    always_ff @(posedge clk) begin
      if( ~nrst ) begin
        ...
        data[WIDTH-1:0] <= dat_in[WIDTH-1:0];
      end
    end
    ...
    if( (TYPE == "MLAB" || TYPE == "M20K") && LENGTH>4 ) begin
      ...
      if( REGISTER_OUTPUTS=="TRUE" ) begin
        assign dat_out[WIDTH-1:0] = reg_out[WIDTH-1:0];
      end else begin
        // avoiding first word fall-through
        assign dat_out[WIDTH-1:0] = (fifo_out_ena)?(fifo_out[WIDTH-1:0]):('0);
      end
      ...
      always_ff @(posedge clk) begin
        ...
        assign dat_out[WIDTH-1:0] = fifo_out[WIDTH-1:0];
      end
      ...
      always_ff @(posedge clk) begin
        ...
        if( ~nrst ) begin
          ...
          data[1][WIDTH-1:0] <= dat_in[WIDTH-1:0];
        end
      end
      assign dat_out[WIDTH-1:0] = data[LENGTH][WIDTH-1:0];
