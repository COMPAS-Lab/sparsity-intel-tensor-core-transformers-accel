...
case class ClosNetSwitch(bitwidth: Int) extends Component {
  val io = new Bundle{
...
case class ClosNet(num_ports: Int, bitwidth: Int, dest_width: Int, asso_tcc_id: Int) extends Component {
  val io = new Bundle {
  ...
  val nConnStgs: Int = log2Up(num_ports) - 1
  val nUnitsStg: Int = (num_ports / 2)

  val switches = Array.fill(nConnStgs + 1, nUnitsStg)(ClosNetSwitch(bitwidth))
